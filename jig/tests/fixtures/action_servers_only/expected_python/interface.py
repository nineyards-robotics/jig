...
@dataclass
class Actions:
    fibonacci: jig.SingleGoalActionServer[Fibonacci, Fibonacci.Goal, Fibonacci.Result, Fibonacci.Feedback]
    math_compute: jig.SingleGoalActionServer[Fibonacci, Fibonacci.Goal, Fibonacci.Result, Fibonacci.Feedback]
...
@dataclass
class ActionServersOnlySession(jig.Session, Generic[SessionT]):
...
T = TypeVar("T", bound=ActionServersOnlySession)


class _ActionServersOnlyNode(jig.BaseNode[T]):
    ...
    def _create_session(self, node) -> T:
        ...
        # initialise actions
        actions = Actions(
            fibonacci=jig.SingleGoalActionServer[Fibonacci, Fibonacci.Goal, Fibonacci.Result, Fibonacci.Feedback](node, Fibonacci, "fibonacci"),
            math_compute=jig.SingleGoalActionServer[Fibonacci, Fibonacci.Goal, Fibonacci.Result, Fibonacci.Feedback](node, Fibonacci, "/math/compute"),
        )
    ...
    def _deactivate_entities(self, sn: T) -> None:
        for timer in sn.timers:
            timer.cancel()
        sn.actions.fibonacci.deactivate()
        sn.actions.math_compute.deactivate()

    def _destroy_entities(self, sn: T) -> None:
        for timer in sn.timers:
            sn.node.destroy_timer(timer)
        sn.timers.clear()
        sn.actions.fibonacci._destroy(sn.node)
        sn.actions.math_compute._destroy(sn.node)
...
def run(
    ...
    *,
    ...
    wrapper = _ActionServersOnlyNode(
        session_type,
        on_configure,
        on_activate=on_activate,
        on_deactivate=on_deactivate,
