...
T = TypeVar("T", bound=QosParamSubstitutionSession)


class _QosParamSubstitutionNode(jig.BaseNode[T]):
    ...
    def _create_session(self, node) -> T:
        ...
        sn.publishers.processed_data._initialise(sn, String, "/processed_data", QoSProfile(history=HistoryPolicy.KEEP_LAST, depth=params.output_queue_depth, reliability=ReliabilityPolicy.RELIABLE))
    ...
    def _destroy_entities(self, sn: T) -> None:
        ...
        sn.timers.clear()
        sn.publishers.processed_data._destroy(sn.node)
        sn.subscribers.sensor_data._destroy(sn.node)
...
def run(
    ...
    *,
    ...
    wrapper = _QosParamSubstitutionNode(
        session_type,
        on_configure,
        on_activate=on_activate,
        on_deactivate=on_deactivate,
