...
@dataclass
class TfStaticBroadcasterOnlySession(jig.Session, Generic[SessionT]):
...
T = TypeVar("T", bound=TfStaticBroadcasterOnlySession)


class _TfStaticBroadcasterOnlyNode(jig.BaseNode[T]):
...
def run(
    ...
    *,
    ...
    wrapper = _TfStaticBroadcasterOnlyNode(
        session_type,
        on_configure,
        on_activate=on_activate,
        on_deactivate=on_deactivate,
