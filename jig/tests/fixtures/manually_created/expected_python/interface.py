...
T = TypeVar("T", bound=ManuallyCreatedSession)


class _ManuallyCreatedNode(jig.BaseNode[T]):
    ...
    def _create_session(self, node) -> T:
        ...
        sn.publishers.auto_topic._initialise(sn, String, "auto_topic", QoSProfile(history=HistoryPolicy.KEEP_LAST, depth=10, reliability=ReliabilityPolicy.RELIABLE))

        # initialise subscribers
        sn.subscribers.auto_sub._initialise(sn, Bool, "auto_sub", QoSProfile(history=HistoryPolicy.KEEP_LAST, depth=10, reliability=ReliabilityPolicy.BEST_EFFORT))
    ...
    def _destroy_entities(self, sn: T) -> None:
        ...
        sn.timers.clear()
        sn.publishers.auto_topic._destroy(sn.node)
        sn.subscribers.auto_sub._destroy(sn.node)
