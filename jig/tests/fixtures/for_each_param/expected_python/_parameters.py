    ...
    class ParamListener:
        ...
        def update(self, parameters):
            ...
            for param in parameters:
                if param.name == self.prefix_ + "managed_nodes":
                    updated_params.managed_nodes = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))

                if param.name == self.prefix_ + "robot_id":
        ...
        def declare_params(self):
            ...
            if not self.node_.has_parameter(self.prefix_ + "managed_nodes"):
                descriptor = ParameterDescriptor(description="List of managed node names", read_only = True)
                parameter = updated_params.managed_nodes
                self.node_.declare_parameter(self.prefix_ + "managed_nodes", parameter, descriptor)

            if not self.node_.has_parameter(self.prefix_ + "robot_id"):
                descriptor = ParameterDescriptor(description="Robot identifier", read_only = True)
                parameter = updated_params.robot_id
                self.node_.declare_parameter(self.prefix_ + "robot_id", parameter, descriptor)

            # TODO: need validation
            # get parameters and fill struct fields
            param = self.node_.get_parameter(self.prefix_ + "managed_nodes")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            updated_params.managed_nodes = param.value
