...
@dataclass
class TfBroadcasterOnlySession(jig.Session):
...
T = TypeVar("T", bound=TfBroadcasterOnlySession)


class _TfBroadcasterOnlyNode(jig.BaseNode[T]):
...
def run(
    ...
    *,
    ...
    wrapper = _TfBroadcasterOnlyNode(
        session_type,
        on_configure,
        on_activate=on_activate,
        on_deactivate=on_deactivate,
